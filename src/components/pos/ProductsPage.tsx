...
export function ProductsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('کیا آپ یہ پراڈکٹ غیر فعال کرنا چاہتے ہیں؟')) return;
  ...
  const handleEdit = (p: Product) => {
    setEditing(p);
    setForm({
      barcode: p.barcode, name: p.name, subCategoryId: p.subCategoryId || '',
      groupId: p.groupId || '', unitId: p.unitId || '',
  ...
  const generateAndShowBarcode = () => {
    if (editing?.barcode && barcodeRef.current) {
      import('jsbarcode').then(JsBarcode => {
        JsBarcode.default(barcodeRef.current, editing.barcode, {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-2xl font-bold">پراڈکٹس مینجمنٹ</h2>
        <Dialog open={open} onOpenChange={o => { setOpen(o); if (!o) { setEditing(null); setForm(emptyProduct); }}}>
          <DialogTrigger asChild>
            <Button className="bg-emerald-600 hover:bg-emerald-700">
              <Plus className="h-4 w-4 mr-2" /> نیا پراڈکٹ
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editing ? 'پراڈکٹ ایڈٹ' : 'نیا پراڈکٹ'}</DialogTitle>
            </DialogHeader>
            <div className="grid gap-3 py-2">
              <div>
                <Label>بارکوڈ</Label>
                <div className="flex gap-2">
                  <Input value={form.barcode} onChange={e => update('barcode', e.target.value)} placeholder="خودکار بنے گا" className="font-mono" />
                  {editing && (
                    <Button variant="outline" size="icon" onClick={generateAndShowBarcode}><Barcode className="h-4 w-4" /></Button>
                  )}
                </div>
                {editing && <svg ref={barcodeRef} className="mt-2 mx-auto" />}
              </div>
              <div>
                <Label>پراڈکٹ نام *</Label>
                <Input value={form.name} onChange={e => update('name', e.target.value)} placeholder="پراڈکٹ کا نام" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label>خریداری قیمت</Label>
                ...
                <div>
                  <Label>تھوک قیمت</Label>
                  <Input type="number" value={form.wholeSalePrice} onChange={e => update('wholeSalePrice', +e.target.value)} />
                </div>
                <div>
                  <Label>اسٹاک</Label>
                ...
                <div>
                  ...
                  <Select value={form.unitId} onValueChange={v => update('unitId', v)}>
                    <SelectTrigger><SelectValue placeholder="یونٹ منتخب کریں" /></SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Select value={form.groupId} onValueChange={v => update('groupId', v)}>
                    <SelectTrigger><SelectValue placeholder="گروپ منتخب کریں" /></SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Select value={form.subCategoryId} onValueChange={v => update('subCategoryId', v)}>
                    <SelectTrigger><SelectValue placeholder="کیٹیگری منتخب کریں" /></SelectTrigger>
                    <SelectContent>
              ...
              <Button className="bg-emerald-600 hover:bg-emerald-700 w-full mt-2" onClick={handleSave} disabled={loading || !form.name}>
                <Save className="h-4 w-4 mr-2" /> {editing ? 'اپ ڈیٹ' : 'محفوظ'}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <div className="relative max-w-md">
        ...
        <Input
          className="pl-10" placeholder="پراڈکٹ نام یا بارکوڈ سیرچ کریں..."
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b">
                  ...
                  <th className="text-right p-3 font-medium">خرید</th>
                  <th className="text-right p-3 font-medium">فروخت</th>
                  <th className="text-right p-3 font-medium">اسٹاک</th>
                  <th className="text-left p-3 font-medium">گروپ</th>
                  <th className="text-center p-3 font-medium">عملیات</th>
                </tr>
              </thead>
              <tbody>
                {products.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-8 text-muted-foreground">کوئی پراڈکٹ نہیں</td></tr>
                ) : products.map(p => (
                  <tr key={p.id} className="border-b hover:bg-slate-50">
                    <td className="p-3 font-mono text-xs">{p.barcode}</td>
                    <td className="p-3 font-medium">{p.name}</td>
                    <td className="p-3 text-right">Rs {p.purchasePrice.toLocaleString()}</td>
                    <td className="p-3 text-right font-semibold text-emerald-600">Rs {p.salePrice.toLocaleString()}</td>
